...
class WebpackAliyunOss {
  ...
  doWithWebpack (compiler) {
    compiler.hooks.afterEmit.tapPromise('WebpackPluginOss', async (compilation) => {
  ...
  async upload (files, inWebpack, outputPath = '') {
    ...
    if (test) {
      console.log(`\nCurrently running in test mode. your files won\'t realy be uploaded.`.green.underline)
    } else {
      console.log(`\nYour files will be uploaded very soon.`.green.underline)
    }
  ...
  checkOptions (options = {}) {
    ...
    if (setVersion && !version) errStr += `\nsetVersion and version must exist together`
